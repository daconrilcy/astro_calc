...
- Scope: the Rust workspace rooted at `C:\dev\astral_calculation\astral_llm`, plus the parent repository workspace rules that still govern package commands from `C:\dev\astral_calculation`.
...
- Primary evidence sources: [../AGENTS.md](../AGENTS.md), [Cargo.toml](Cargo.toml), [README.md](README.md), and [../.audit/audit-1782104887.md](../.audit/audit-1782104887.md).

## Project Map

- Current nested workspace members from [Cargo.toml](Cargo.toml):
...
- `astral_llm_worker` exists under `crates/` in this tree, but it is not listed in the nested workspace manifest. Treat it as a separate runtime unless the manifest is intentionally changed. Evidence: [Cargo.toml](Cargo.toml) and the audit.
- Composition roots:
...
- Use the parent repository workspace as the normal Cargo entry point for package commands from `C:\dev\astral_calculation`; keep `astral_llm/` inspectable and locally runnable. Evidence: [../AGENTS.md](../AGENTS.md), [README.md](README.md), and [Cargo.toml](Cargo.toml).
- Keep `astral_llm_domain` free of application, infra, API, and worker dependencies. Evidence: [crates/astral_llm_domain/src/lib.rs](crates/astral_llm_domain/src/lib.rs) and the audit.
- Decouple `astral_llm_application` from `astral_llm_infra` through application-owned ports, traits, and DTOs. Evidence: the audit found direct infra imports in `generate_reading_use_case`, `provider_factory`, `provider_router`, `prompt_trace`, and `raw_provider_trace`.
- Treat DB rows as canonical for configurable product, catalog, and policy data; keep protocol identifiers, Serde field names, enum variants, feature flags, and technical invariants in code. Evidence: [../AGENTS.md](../AGENTS.md) and the audit finding on canonical-data hard-coding in `crates/astral_llm_infra/src/evidence_canonical.rs`.
- Freeze only public, persisted, or externally consumed JSON contracts. Internal orchestration DTOs and provider scratch payloads may change when guarded by characterization tests. Evidence: [README.md](README.md), [../contracts/](../contracts/), and the audit.
- Keep fail-fast behavior at binary boundaries only; internal boot helpers should return typed errors instead of `panic!`, `expect()`, or `unwrap()` for expected config, DB, or provider failures. Evidence: [crates/astral_llm_api/src/main.rs](crates/astral_llm_api/src/main.rs), [crates/astral_llm_worker/src/main.rs](crates/astral_llm_worker/src/main.rs), and the audit.
- Prefer canonical module imports and remove deprecated root aliases after caller inventory. Do not add new root-level `pub use` items unless they are intended as stable API. Evidence: the broad export surfaces in [crates/astral_llm_application/src/lib.rs](crates/astral_llm_application/src/lib.rs) and [crates/astral_llm_domain/src/lib.rs](crates/astral_llm_domain/src/lib.rs).
- Treat provider traces as observability/debug data, not domain evidence. Evidence: audit notes on `prompt_trace` and `raw_provider_trace`.
- Put integration and characterization tests under root `tests/` by default. Evidence: the audit verified that production `src/` files currently do not contain inline `#[cfg(test)]` modules or inline `#[test]` functions.
- Split verification into mandatory compile/tests and environment-dependent smoke checks.

## Non-Negotiable Invariants

- Treat the parent repository workspace as the canonical Cargo entry point for normal commands, including `cargo run -p astral_llm_worker` and package tests. The nested [Cargo.toml](Cargo.toml) exists so `astral_llm/` remains inspectable and locally runnable, but the parent [../Cargo.toml](../Cargo.toml) is the source of truth for cross-crate membership and dependency wiring. Evidence: [../Cargo.toml](../Cargo.toml), [Cargo.toml](Cargo.toml), [README.md](README.md), and [../AGENTS.md](../AGENTS.md).
...
- For low-risk facade trimming, prefer aliases whose caller inventory is empty or limited to one local test file. The 2026-06-22 inventory for `editorial_validation::{EditorialFixtureSpec, EditorialValidator}` found a single test caller in [../tests/astral_llm_editorial_fixtures.rs](../tests/astral_llm_editorial_fixtures.rs), so the canonical module path became the required import before removing the root alias. Evidence: repository-wide `rg -n "EditorialFixtureSpec|EditorialValidator" astral_llm tests`.
...
- Trace helpers in `astral_llm_application` must receive trace behavior through injected settings or application-owned DTOs; do not move `env_var`, `env_bool`, `ASTRAL_LLM_*`, or other environment reads into `prompt_trace`, `raw_provider_trace`, or sibling application helpers. Evidence: [crates/astral_llm_application/src/prompt_trace.rs](crates/astral_llm_application/src/prompt_trace.rs), [crates/astral_llm_application/src/raw_provider_trace.rs](crates/astral_llm_application/src/raw_provider_trace.rs), and the "Next Closed Slice" in [../.audit/audit-1782104887.md](../.audit/audit-1782104887.md).
- Composition roots own the env-to-trace-settings mapping. `astral_llm_api/src/main.rs` and `astral_llm_worker/src/main.rs` may translate `ASTRAL_LLM_LOG_COMPILED_PROMPTS`, `ASTRAL_LLM_PROMPT_LOG_DIR`, `ASTRAL_LLM_STORE_RAW_PROVIDER_OUTPUTS`, and `ASTRAL_LLM_RAW_PROVIDER_OUTPUT_DIR` into application trace settings, but `astral_llm_application` helper modules must stay on the settings side of that boundary. Evidence: [crates/astral_llm_api/src/main.rs](crates/astral_llm_api/src/main.rs), [crates/astral_llm_worker/src/main.rs](crates/astral_llm_worker/src/main.rs), [crates/astral_llm_infra/src/config.rs](crates/astral_llm_infra/src/config.rs), and [../.plan/plan-1782105282.md](../.plan/plan-1782105282.md).
- Keep trace-setting default/override assembly centralized in `PromptTraceSettings::from_runtime` and `RawProviderTraceSettings::from_runtime`; API and worker entrypoints should call those constructors rather than manually mutating fields inline. Evidence: [crates/astral_llm_application/src/prompt_trace.rs](crates/astral_llm_application/src/prompt_trace.rs), [crates/astral_llm_application/src/raw_provider_trace.rs](crates/astral_llm_application/src/raw_provider_trace.rs), [crates/astral_llm_api/src/main.rs](crates/astral_llm_api/src/main.rs), and [crates/astral_llm_worker/src/main.rs](crates/astral_llm_worker/src/main.rs).
...
- For the current refactor backlog, the latest audit explicitly called out the remaining dependency-direction problem, oversized orchestrators, broad root exports, and canonical-data hard-coding. Use this as the acceptance filter before editing application or infra internals. Evidence: [../.audit/audit-1782104887.md](../.audit/audit-1782104887.md).
- For the Phase 1 trace-settings slice, verify both the direct env-read metric and the runtime-settings behavior with:
