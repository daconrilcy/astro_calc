...
mod tests {
    ...
    #[test]
    fn formats_french_placement_label() {
        let catalog = test_catalog();
        let h = AstroLabelHumanizer::new(&catalog);
        assert_eq!(
            h.placement_label("fr", "sun", "capricorn", Some(2)),
            "Soleil en Capricorne en maison 2"
        );
    }

    #[test]
    fn humanizes_signal_moon_in_french() {
        let catalog = test_catalog();
        let h = AstroLabelHumanizer::new(&catalog);
        let fact = NormalizedAstroFact {
        ...
        assert_eq!(
            ...
            "Lune en Poissons en maison 4"
        );
    }

    #[test]
    fn humanizes_signal_moon_from_evidence_block() {
        let catalog = test_catalog();
        let h = AstroLabelHumanizer::new(&catalog);
        let fact = NormalizedAstroFact {
            id: "signal:object_position:moon".into(),
    ...
    #[test]
    fn humanizes_signal_angle_ascendant_in_french() {
        let catalog = test_catalog();
        let h = AstroLabelHumanizer::new(&catalog);
        let fact = NormalizedAstroFact {
            id: "signal:angle:ascendant:sign:scorpio".into(),
            kind: AstroFactKind::Angle,
            kind_code: "angle".into(),
            usage: AstroFactUsage::InterpretiveBasis,
            label: "Ascendant in Scorpio".into(),
            value: serde_json::json!({ "title": "Ascendant in Scorpio" }),
            interpretive_weight: None,
            domains: vec![],
        };
        assert_eq!(
