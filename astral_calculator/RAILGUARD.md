...
- Any reduction of inherent repository wrappers in `src/infra/db/reference_repository.rs` or `src/infra/db/calculation_repository.rs` must start from a direct-caller inventory and remove only one coherent wrapper family at a time. Retained wrappers need caller evidence; do not delete or keep them speculatively. Evidence: `.audit/audit-1782066159.md`, `astral_calculator/src/infra/db/reference_repository.rs`, `astral_calculator/src/infra/db/calculation_repository.rs`, `astral_calculator_http/src/reference_status.rs`, and `astral_calculator/src/runtime/mod.rs`.
