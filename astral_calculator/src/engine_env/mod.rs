...
mod tests {
    ...
    #[test]
    fn maps_zodiac_id_from_json_db_seed() {
        std::env::set_var("ASTRAL_ZODIACAL_REFERENCE_SYSTEM_ID", "1");
        std::env::remove_var("ASTRAL_ZODIACAL_REFERENCE_SYSTEM");
        assert_eq!(
            zodiacal_reference_system_key_from_env().unwrap(),
            "tropical"
        );
        std::env::remove_var("ASTRAL_ZODIACAL_REFERENCE_SYSTEM_ID");
    }

    #[test]
    fn key_env_takes_precedence_over_id_for_audit_ids() {
        std::env::set_var("ASTRAL_ZODIACAL_REFERENCE_SYSTEM", "tropical");
        std::env::set_var("ASTRAL_ZODIACAL_REFERENCE_SYSTEM_ID", "99");
        assert_eq!(zodiacal_reference_system_id_from_env().unwrap(), 1);
        std::env::remove_var("ASTRAL_ZODIACAL_REFERENCE_SYSTEM");
        std::env::remove_var("ASTRAL_ZODIACAL_REFERENCE_SYSTEM_ID");
    }

    #[test]
    fn rejects_partial_birth_env_triplet() {
        std::env::set_var("ASTRAL_BIRTH_DATE", "1990-01-02");
        std::env::set_var("ASTRAL_BIRTH_TIME", "03:04:05");
        std::env::remove_var("ASTRAL_BIRTH_TIMEZONE");
        std::env::remove_var("ASTRAL_BIRTH_DATETIME_UTC");

        let error = birth_fields_from_env().unwrap_err().to_string();
        assert!(error.contains("must all be set together"));

        std::env::remove_var("ASTRAL_BIRTH_DATE");
        std::env::remove_var("ASTRAL_BIRTH_TIME");
    }

    #[test]
    fn rejects_invalid_projection_level() {
        std::env::set_var("ASTRAL_PROJECTION_LEVEL", "verbose");
        let error = projection_level_from_env().unwrap_err().to_string();
        assert!(error.contains("ASTRAL_PROJECTION_LEVEL"));
        std::env::remove_var("ASTRAL_PROJECTION_LEVEL");
    }
}
