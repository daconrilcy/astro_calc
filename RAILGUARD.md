...
- Product features in `astral_calculator` are orchestrators over contracts and shared calculations. They must not import another feature’s internals. Evidence: `AGENTS.md`, `tests/refactor_governance_tests.rs`, `.audit/audit-1782055817.md`.
- Canonical referential data must come from the database path when possible; do not add new hard-coded production reference catalogs or runtime fixture fallbacks. Evidence: `AGENTS.md`, `.audit/audit-1782055817.md`, `astral_calculator/RAILGUARD.md`.
...
- Keep behavior and scenario tests under the root `tests/` tree; do not move broad test coverage into production `src/**/*.rs`. Evidence: `AGENTS.md`, `.audit/audit-1782055817.md`, `astral_calculator/Cargo.toml`.
...
- Preserve existing focused error boundaries; do not replace them with ad hoc panics or broad dynamic error handling in runtime paths. Evidence: `.audit/audit-1782055817.md`, `astral_calculator/RAILGUARD.md`, governance tests forbidding panic-prone horoscope paths.
...
- Prefer normal Rust module layout over `#[path]` indirection for in-crate modules; any remaining `#[path]` usage needs explicit justification. Evidence: `.audit/audit-1782055817.md`.
...
- inspect `.audit/audit-1782055817.md` when the work touches current calculator boundary findings
