...
Push-Location $crateDir
try {
    $schemaEnv = Save-Env @("NATAL_V10_SCHEMA_PAYLOAD_PATH")
    try {
        $env:NATAL_V10_SCHEMA_PAYLOAD_PATH = $payloadUnderTestPath
        cargo test --quiet --test contract_basic_v8_tests external_payload_matches_json_schema_v10_when_requested
        if ($LASTEXITCODE -ne 0) {
...
if ($generatedProjection -ne $goldenProjection) {
    ...
    throw "Generated natal v10 projection differs from golden. See $generatedOut and $goldenOut."
}

Write-Host "Generated natal v10 projection matches golden."
