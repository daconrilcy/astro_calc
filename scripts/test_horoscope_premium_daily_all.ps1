<#
.SYNOPSIS
    Lance la suite complete de validation du service horoscope_premium_daily_local_2h_slots.

.EXAMPLE
    .\scripts\test_horoscope_premium_daily_all.ps1

.EXAMPLE
    .\scripts\test_horoscope_premium_daily_all.ps1 -BaseUrl http://127.0.0.1:8081 -CalculatorUrl http://127.0.0.1:8080
#>
param(
...
try {
    ...
    if (-not $SkipSmoke) {
        ...
        Invoke-Step "Horoscope Premium Daily fake smoke" {
            & (Join-Path $repoRoot "scripts\test_horoscope_premium_daily_fake.ps1") `
                -BaseUrl $BaseUrl `
                -CalculatorUrl $CalculatorUrl
        }
    }
