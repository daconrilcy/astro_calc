...
- `.audit/audit-1782059752.md`: latest refactor audit for `astral_calculator`; use it as the primary evidence source for current boundary risks and next refactor slices.
...
- Use the latest audit to drive the next refactor slice: it still flags broad compatibility exports, duplicated chart-context loading, porous JSON boundaries, and repository forwarding boilerplate. Those are documented constraints for upcoming plan and dev phases, not optional cleanup. Evidence: `.audit/audit-1782059752.md`.
...
- Avoid introducing new `serde_json::Value` carriers in domain/application code when a typed record can own the data. The latest audit still identifies JSON-shaped state in `domain` and `application` as a refactor risk. Evidence: `.audit/audit-1782059752.md`.
