...
$validate = Invoke-AstralJson -Method Post -Uri "$CalculatorUrl/v1/internal/calculations/validate" -Headers $headers -Body $validateBody
if ($validate.valid -ne $true) { throw "Calculator validation failed" }
Write-Host "OK POST /v1/internal/calculations/validate"
...
Write-Host "OK POST /v1/internal/calculations/natal chart_calculation_id=$chartCalculationId"

$simplified = Invoke-AstralJson -Method Post -Uri "$CalculatorUrl/v1/internal/calculations/natal/simplified" `
...
Write-Host "OK POST /v1/internal/calculations/natal/simplified"

$horoscopeRequest = New-E2EHoroscopeCalculationRequest -RepoRoot $repoRoot -ChartCalculationId $chartCalculationId
$horoscope = Invoke-AstralJson -Method Post -Uri "$CalculatorUrl/v1/internal/calculations/horoscope/daily-natal" `
...
Write-Host "OK POST /v1/internal/calculations/horoscope/daily-natal"
