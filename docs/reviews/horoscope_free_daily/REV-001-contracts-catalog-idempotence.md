...
|---|---|---|---|---|
| FREE-001 | Major | `horoscope_response` etait verrouille sur le service Basic et ne pouvait pas valider une forme Free sans `slots`. | Schema passe en variantes conditionnelles par `service_code`. | fixed |
| FREE-002 | Major | Le payload Free risquait de dupliquer le payload Basic. | Ajout de `horoscope_daily_natal_request` commun pour le service Free, en conservant le contrat Basic historique. | fixed |
