...
It is updated from the latest refactor audit at `.audit/audit-1782064372.md`.
...
- Audit-backed constants rule: do not leave canonic business codes hard-coded in product or domain logic when the value can come from the database. The current audit names `placidus`, `whole_sign`, `equal`, `porphyry`, the `se-2026a` ephemeris code, the `en` language code, and the `tropical` JSON value as the first items to remove or externalize. Evidence: `.audit/audit-1782064372.md`, `astral_calculator/src/astrology/ephemeris.rs`, `astral_calculator/src/domain/natal_input.rs`, `astral_calculator/src/features/natal/application/natal_calculation_service.rs`, `astral_calculator/src/features/horoscope/daily.rs`.
...
- The typed position-context Phase 1 slice is closed only while `astrology/ephemeris.rs` uses `PositionFactContext::{from_calculated_position,from_angle_position}` instead of inline `facts_json` assembly, and `features/natal/payload/build/house_axes.rs` reads angle/object metadata through typed `ObjectPositionFact` helpers rather than `facts_json.get(...)`. Evidence: `astral_calculator/src/domain/chart_facts.rs`, `astral_calculator/src/astrology/ephemeris.rs`, `astral_calculator/src/features/natal/payload/build/house_axes.rs`, `.audit/implementation-audit-1782065395.md`.
...
- `astral_calculator/src/features/horoscope/application/horoscope_service.rs` should not keep duplicating chart-calculation runtime loading between daily and period flows. Extract a small private helper such as `load_horoscope_runtime_context` for the shared steps named in the latest audit: `chart_calculation_id` parsing, positions/input/context loading, supported-object checks, theme mappings, transit calculation, and max-orb resolution. Evidence: `.audit/audit-1782064372.md`, `astral_calculator/src/features/horoscope/application/horoscope_service.rs`.
- `astral_calculator/src/features/horoscope/daily.rs` and `astral_calculator/src/astrology/ephemeris.rs` must keep large orchestration functions decomposed into small private helpers named by intention. The audit specifically calls out `derived_slot` and `calculate_chart` as too large; future edits in those files should prefer resolution, snapshot construction, output mapping, and validation helpers over additional inline branching. Evidence: `.audit/audit-1782064372.md`.
...
- Preserve the current public module shape for compatibility, but do not add new broad root exports. The audit still flags the root surface as wide, so new code should use canonical module paths instead of inventing fresh `pub mod` aliases. Evidence: `astral_calculator/src/lib.rs`, `.audit/audit-1782064372.md`, and `tests/deprecated_root_alias_compat_tests.rs`.
...
- The current audit still flags broad compatibility exports, duplicated chart-context loading, hard-coded business constants, and repository forwarding boilerplate in `astral_calculator`. The production-owned natal catalog fallback is closed in this workspace slice and must not be reintroduced.
