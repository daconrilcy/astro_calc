...
- Evidence base: [../AGENTS.md](../AGENTS.md), [Cargo.toml](Cargo.toml), [README.md](README.md), [../.audit/audit-1782120026.md](../.audit/audit-1782120026.md), [../.audit/audit-1782116301.md](../.audit/audit-1782116301.md), and [../.audit/implementation-audit-1782115488.md](../.audit/implementation-audit-1782115488.md).
...
- Current closed slice from the user prompt: application-side hashing only. `astral_llm_application` must stop importing `astral_llm_infra::hash_json` from feature/runtime modules, while preserving persisted hashes and JSON contracts. Evidence: [../.audit/audit-1782120026.md](../.audit/audit-1782120026.md), [crates/astral_llm_application/src/generate_reading_use_case.rs](crates/astral_llm_application/src/generate_reading_use_case.rs), and [crates/astral_llm_application/src/horoscope/mod.rs](crates/astral_llm_application/src/horoscope/mod.rs).
...
- For the current hash-boundary slice, prefer an application-owned helper such as `reading_persistence::hash_json_value(&serde_json::Value) -> String` over direct infra hashing imports in runtime modules. Use the same algorithm as the infra helper (`serde_json::to_vec(...).unwrap_or_default()`, SHA-256, hex) so persisted hashes stay stable. Evidence: [../.audit/audit-1782120026.md](../.audit/audit-1782120026.md), [crates/astral_llm_application/src/reading_persistence.rs](crates/astral_llm_application/src/reading_persistence.rs), and [crates/astral_llm_infra/src/persistence.rs](crates/astral_llm_infra/src/persistence.rs).
...
- For the current hash helper slice, the measurable invariant is narrow: `rg` must no longer find `astral_llm_infra::hash_json` imported from `generate_reading_use_case.rs` or `horoscope/mod.rs`, and the resulting persisted hashes must remain deterministic. Do not widen this slice into catalog, provider-factory, runtime-composition, or persistence-shape changes. Evidence: [../.audit/audit-1782120026.md](../.audit/audit-1782120026.md) and the user-provided scope note for `generate_reading_use_case.rs`, `horoscope/mod.rs`, and `reading_persistence.rs`.
...
- The hash boundary slice is intentionally narrower than the broader catalog or persistence refactors. Do not opportunistically move `RunPersistence`, `SharedCanonicalCatalog`, provider factory wiring, or runtime composition helpers in the same change set unless the task is re-scoped and re-documented first.
...
- The hash boundary slice must preserve deterministic persisted values. If a later audit finds a drift between `reading_persistence::hash_json_value` and the infra helper algorithm, treat that as a compatibility regression, not just an internal cleanup opportunity.
