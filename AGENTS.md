...
- **E2E lecture longue** : `.\scripts\test_natal_premium_plus_profile.ps1` (genere + valide 9 chapitres ; seuils domaine 520 mots / 6 basis, synthesis 520 mots / 4 basis depuis `natal_premium_plus.json` ; `-SkipGenerate` pour revalider une sortie existante). Wrapper : `.\scripts\generate_premium_plus_reading_e2e.ps1` (timeout script 1800 s). Comparaison baseline : `.\scripts\compare_premium_plus_versions.ps1`. Fixture `request-premium-plus-rich.json`, `domain_count: 8`. Requis `.env` : `ASTRAL_LLM_REQUEST_TIMEOUT_MS=900000`, `OPENAI_API_KEY`.
