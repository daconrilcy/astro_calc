...
Push-Location $crateDir
try {
    $schemaEnv = Save-Env @("NATAL_V11_SCHEMA_PAYLOAD_PATH")
    try {
        $env:NATAL_V11_SCHEMA_PAYLOAD_PATH = $payloadUnderTestPath
        cargo test --quiet --test contract_basic_v8_tests external_payload_matches_json_schema_v11_when_requested
        if ($LASTEXITCODE -ne 0) {
