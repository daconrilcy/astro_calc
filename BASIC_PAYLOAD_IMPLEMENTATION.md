...
- Hardened `.dockerignore` so Docker builds exclude `tmp_target/`, nested `tmp_target/`, `.cargo-target-*`, and local audit/fix/plan scratch directories. Runtime-required assets (`contracts/`, prompts, source crates, configs) remain available to the images.
- Shared the Docker BuildKit release target cache across `astral_calculator_http`, `astral_llm_api`, `astral_llm_worker`, and `astral_gateway` with `id=astral-target-release`; `sharing=locked` is preserved so parallel Compose builds do not write the same Cargo target directory concurrently.
- Added per-step duration logging to `scripts/docker_update_integration_stack.ps1` so future slow phases are visible immediately instead of being hidden behind the global wrapper.
- Fixed the subsequent host-side `astral_calculator_http_tests` failure: the wrapper now includes `docker-compose.dev-db-port.yml` by default and checks the host PostgreSQL TCP endpoint before running Rust checks. `-SkipHostDbPort` keeps the old no-host-port behavior for Docker-only runs.
- Realigned the horoscope supported-object reference table with the runtime contract: `json_db/horoscope_supported_objects.json` now exposes `object_code`, `is_enabled`, and `weight`; the calculator query consumes those columns directly; a governance test prevents the stale `is_enabled_v1` seed shape from returning.
- Fixed the follow-up gateway smoke failure `HOROSCOPE_SCORING_FAILED`: the LLM horoscope reference loader now accepts the current `is_enabled` flag while remaining compatible with legacy `is_enabled_v1` seeds such as supported aspects.
- Kept the host DB port override active inside the fake-provider E2E helpers so their `--force-recreate` calls do not silently recreate Postgres without `localhost:${POSTGRES_PORT}`.
- Invariants: no service contract change; no branch; no runtime DB/reference behavior change; Docker builds still use the root Cargo workspace and `tmp_target` from `.cargo/config.toml`; host Cargo tests still use `.env` `DATABASE_URL`.
- Verification: static inspection of `.dockerignore`, Dockerfiles, and script flow; targeted Compose/PowerShell validation; `cargo test -p astral_calculator --test refactor_governance_tests horoscope_supported_objects_seed_matches_runtime_query_contract`; `cargo test -p astral_calculator --test refactor_governance_tests horoscope_signal_theme_mappings_seed_matches_runtime_query_contract`; `cargo test -p astral_llm_api --test horoscope_tests daily_scoring_accepts_current_supported_object_seed_contract`; `cargo test -p astral_calculator_http --test astral_calculator_http_tests`; `docker compose -f docker-compose.yml -f docker-compose.dev-db-port.yml build astral_llm_api astral_llm_worker`; `.\scripts\docker_update_integration_stack.ps1 -SkipBuild -SkipRustChecks`.
